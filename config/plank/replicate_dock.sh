...
if [ $MONITORS -gt 1 ]; then
    ...
    # Needed if the primary monitor is not the default laptop screen in multimonitor setups
    ...
    monitor_names_arr=($monitor_names)
	for i in $(seq 2 $MONITORS); do
		...
		# Set the monitor
		index=$((i - 2))
		dconf write /net/launchpad/plank/docks/dock$i/monitor "'${monitor_names_arr[index]}'"
